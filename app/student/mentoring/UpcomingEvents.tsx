  ...
  return (
    <div className='bg-white font-satoshi border border-backgroundGray p-5 rounded-md flex flex-col gap-y-12'>
        <div className='self-center flex flex-col items-center'>
            ...
            <Link href={`/student/events`} className='font-medium hover:underline text-primary'>
                View All
            </Link>
        </div>
        <div className='flex flex-col gap-y-4'>
            {events?.slice(0, 3).map(event => (
                <div key={event.title} className='bg-primaryLight2 p-4 flex flex-col gap-y-4 rounded-md'>
                    ...
                    <Link href={`/student/events/${event.id}`} className='border border-backgroundGray rounded-md py-1.5 px-3 bg-white font-bold w-fit self-end'>Explore</Link>
                </div>
            ))}
        </div>
    </div>
